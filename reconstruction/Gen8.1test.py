...
def show_debug_pipeline(img_orig, img_bg_subtracted, mask_glare, img_fringes):
    ...
    axs[1, 1].set_title('4. Fringes (Glass Wall Cropped)')
...
def process_ferrofluid_image(image_path, json_path, projector_angle, px_per_mm):
    ...
    # --- NEW: SOFTWARE ROI CROP (Eliminates Glass Petri Dish Walls) ---
    # Calculates a circular safe zone in the center of the image.
    # Radius is set to 85% of the distance to the border (ignoring outer 15%).
    center_x, center_y = img_width // 2, img_height // 2
    safe_radius = int(min(center_x, center_y) * 0.85) 
    
    roi_mask = np.zeros_like(img)
    cv2.circle(roi_mask, (center_x, center_y), safe_radius, 255, -1)
    
    # Apply the cookie-cutter mask to delete the bent wall lines
    binary_clean = cv2.bitwise_and(binary_clean, binary_clean, mask=roi_mask)
    ...
    # NEW: Tell the keystone math where the new "Safe Zone" boundaries are
    safe_left_bound = center_x - safe_radius
    safe_right_bound = center_x + safe_radius
    floor_margin = max(10, int((safe_right_bound - safe_left_bound) * 0.05)) 
    ...
    for c in contours:
        ...
        # The keystone edge_mask now looks exclusively at the extreme edges of the SAFE ZONE
        edge_mask = (x_coords < safe_left_bound + floor_margin) | (x_coords > safe_right_bound - floor_margin)
