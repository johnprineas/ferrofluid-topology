...
def show_debug_pipeline(img_orig, img_bg_subtracted, mask_glare, img_fringes):
    ...
    axs[1, 0].set_title('3. Glare Mask (Force Disabled for Test Image)')
...
def process_ferrofluid_image(image_path, json_path, projector_angle, px_per_mm):
    ...
    # REPLACEMENT: Force a totally blank mask (zeros) so the synthetic lines aren't deleted.
    mask_glare = np.zeros_like(img)
